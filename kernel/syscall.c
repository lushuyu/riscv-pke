...
ssize_t sys_user_exit(uint64 code) {
    ...
    for (int i = NPROC; i >= 0; i--)
        if (BLOCKED == procs[i].status && &procs[i] == current->parent) {
...
uint64 sys_user_allocate_page() {
    ...
    if (current->user_heap.free_pages_count > 0) {
        ...
        // otherwise, allocate a new page (this increases the size of the heap
        // by one page)
...
uint64 sys_user_free_page(uint64 va) {
    ...
    current->user_heap
        .free_pages_address[current->user_heap.free_pages_count++] = va;
    return 0;
}
...
ssize_t sys_user_wait(uint64 pid) {
    ...
    if (pid == -1) {
        ...
        if (procs[pid].parent == parent) {
            ...
            panic("Syscall: sys_user_wait parent invaild");
            return -1;
        }
    }
    return -1;
