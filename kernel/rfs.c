...
struct vinode *rfs_create(struct vinode *parent, struct dentry *sub_dentry) {
    ...
    // (free_dinode) of a new file being created. hint:  members of free_dinode
    // to be filled are: size, should be zero for a new file. type, see
    // kernel/rfs.h and find the type for a rfs file. nlinks, i.e., the number
    // of links. blocks, i.e., its block count. Note: DO NOT DELETE CODE BELOW
    // PANIC.
    free_dinode->size = 0;
...
int rfs_link(struct vinode *parent, struct dentry *sub_dentry,
    ...
    // whose vfs inode is "link_node". To do that, we need first to know the
    // name of the new (link) file, and then, we need to increase the link count
    // of the existing file. Lastly, we need to make the changes persistent to
    // disk. To know the name of the new (link) file, you need to stuty the
    // structure of dentry, that contains the name member; To incease the link
    // count of the existing file, you need to study the structure of vfs inode,
    // since it contains the inode information of the existing file.
    //
    // hint: to accomplish this experiment, you need to:
    // 1) increase the link count of the file to be hard-linked;
    // 2) append the new (link) file as a dentry to its parent directory; you
    // can use
    //    rfs_add_direntry here.
    // 3) persistent the changes to disk. you can use rfs_write_back_vinode
    // here.
    //
    sub_dentry->dentry_inode->nlinks++;
    if (rfs_add_direntry(parent, sub_dentry->name,
                         sub_dentry->dentry_inode->inum) == -1) {
        panic("Rfs_link: rfs_add_direntry failed");
        return -1;
    }
    if (rfs_write_back_vinode(sub_dentry->dentry_inode) != 0) {
        panic("Rfs_link: rfs_write_back_vinode failed");
        return -1;
    }
    return 0;
}
...
int rfs_unlink(struct vinode *parent, struct dentry *sub_dentry,
    ...
    for (delete_index = 0; delete_index < total_direntrys; ++delete_index) {
        ...
        if (delete_index % one_block_direntrys == 0) {
            rfs_r1block(rdev,
                        parent->addrs[delete_index / one_block_direntrys]);
            p_direntry = (struct rfs_direntry *)rdev->iobuffer;
        }
        if (strcmp(p_direntry->name, sub_dentry->name) == 0) { // found
            break;
...
int rfs_readdir(struct vinode *dir_vinode, struct dir *dir, int *offset) {
    ...
    // the *offset, and used p_direntry to point it. in the remaining
    // processing, we need to return our discovery. the method of returning is
    // to popular proper members of "dir", more specifically, dir->name and
    // dir->inum. note: DO NOT DELETE CODE BELOW PANIC.
...
struct vinode *rfs_mkdir(struct vinode *parent, struct dentry *sub_dentry) {
    ...
    if (free_dinode == NULL)
        panic("rfs_mkdir: no more free disk inode, we cannot create "
              "directory.\n");
