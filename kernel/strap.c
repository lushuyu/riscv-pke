...
static void handle_syscall(trapframe *tf) {
    // tf->epc points to the address that our computer will jump to after the
    // trap handling. for a syscall, we should return to the NEXT instruction
    // after its handling. in RV64G, each instruction occupies exactly 32 bits
    // (i.e., 4 Bytes)
    tf->epc += 4;

    // TODO (lab1_1): remove the panic call below, and call do_syscall (defined
    // in kernel/syscall.c) to conduct real operations of the kernel side for a
    ...
    (tf->regs).a0 =
        do_syscall(syscall_num, (tf->regs).a1, (tf->regs).a2, (tf->regs).a3,
...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval) {
    ...
    case CAUSE_STORE_PAGE_FAULT:
        ...
        user_vm_map(current->pagetable, stval / (PGSIZE) * (PGSIZE), PGSIZE,
                    (uint64)(pag), prot_to_type(PROT_WRITE | PROT_READ, 1));
        break;
    default:
...
void rrsched() {
    ...
    if (current->tick_count >= TIME_SLICE_LEN - 1) {
