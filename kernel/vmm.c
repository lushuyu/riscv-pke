...
int map_pages(pagetable_t page_dir, uint64 va, uint64 size, uint64 pa,
    ...
    for (first = ROUNDDOWN(va, PGSIZE), last = ROUNDDOWN(va + size - 1, PGSIZE);
        ...
        if ((pte = page_walk(page_dir, first, 1)) == 0)
            return -1;
        if (*pte & PTE_V)
            panic("map_pages fails on mapping va (0x%lx) to pa (0x%lx)", first,
                  pa);
    ...
    for (int level = 2; level > 0; level--) {
        ...
        if (*pte & PTE_V) { // PTE valid
            ...
            if (alloc && ((pt = (pte_t *)alloc_page(1)) != 0)) {
                memset(pt, 0, PGSIZE);
                // writes the physical address of newly allocated page to pte,
                // to establish the page table tree.
                *pte = PA2PTE(pt) | PTE_V;
            } else // returns NULL, if alloc == 0, or no more physical page
                   // remains
                return 0;
        }
    }
...
void kern_vm_init(void) {
    ...
    // kernel. this is important when kernel needs to access the memory content
    // of user's app without copying pages between kernel and user spaces.
...
void *user_va_to_pa(pagetable_t page_dir, void *va) {
    ...
    // "page_dir", to locate the PTE that maps "va". If found, returns the "pa"
    // by using: pa = PYHS_ADDR(PTE) + (va & (1<<PGSHIFT -1)) Here, PYHS_ADDR()
    // means retrieving the starting address (4KB aligned), and (va &
    // (1<<PGSHIFT -1)) means computing the offset of "va" inside its page.
    // Also, it is possible that "va" is not mapped at all. in such case, we can
    // find invalid PTE, and should return NULL.
...
void user_vm_unmap(pagetable_t page_dir, uint64 va, uint64 size, int free) {
    ...
    // zero. basic idea here is to first locate the PTEs of the virtual pages,
    // and then reclaim (use free_page() defined in pmm.c) the physical pages.
    // lastly, invalidate the PTEs. as naive_free reclaims only one page at a
    // time, you only need to consider one page to make user/app_naive_malloc to
    // behave correctly.
