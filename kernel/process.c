...
process *alloc_process() {
    ...
    user_vm_map((pagetable_t)procs[i].pagetable, USER_STACK_TOP - PGSIZE,
    ...
    sprint("in alloc_proc. user frame 0x%lx, user stack 0x%lx, user kstack "
           "0x%lx \n",
...
int free_process(process *proc) {
    // we set the status to ZOMBIE, but cannot destruct its vm space
    // immediately. since proc can be current process, and its user kernel stack
    // is currently in use! but for proxy kernel, it (memory leaking) may NOT be
    // a really serious issue, as it is different from regular OS, which needs
    // to run 7x24.
    proc->status = ZOMBIE;
...
int do_fork(process *parent) {
    ...
    for (int i = 0; i < parent->total_mapped_region; i++) {
        ...
        case STACK_SEGMENT:
            memcpy(
                (void *)lookup_pa(child->pagetable,
        ...
        case HEAP_SEGMENT:
            ...
            // convert free_pages_address into a filter to skip reclaimed blocks
            // in the heap when mapping the heap blocks

            memset(free_block_filter, 0, MAX_HEAP_PAGES);
            uint64 heap_bottom = parent->user_heap.heap_bottom;
            for (int i = 0; i < parent->user_heap.free_pages_count; i++) {
                int index =
                    (parent->user_heap.free_pages_address[i] - heap_bottom) /
                    PGSIZE;
            ...
            for (uint64 heap_block = current->user_heap.heap_bottom;
                 heap_block < current->user_heap.heap_top;
                 heap_block += PGSIZE) {
        ...
        case CODE_SEGMENT:
            ...
            // parent's code segment. hint: the virtual address mapping of code
            // segment is tracked in mapped_info page of parent's process
            // structure. use the information in mapped_info to retrieve the
            // virtual to physical mapping of code segment. after having the
            // mapping information, just map the corresponding virtual address
            // region of child to the physical pages that actually store the
            // code segment of parent process. DO NOT COPY THE PHYSICAL PAGES,
            // JUST MAP THEM.
            parent_va = parent->mapped_info[i].va;
            parent_pa = lookup_pa(parent->pagetable, parent_va);
        ...
        case DATA_SEGMENT: {
            ...
            user_vm_map((pagetable_t)child->pagetable,
                        parent->mapped_info[DATA_SEGMENT].va, PGSIZE,
                        (uint64)alloc_page(),
                        prot_to_type(PROT_WRITE | PROT_READ, 1));
